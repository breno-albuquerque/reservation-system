...
using RerservationSystem.Core.Shared.Extensions;
...
namespace RerservationSystem.Core.Features.RegisterUser.Contract
{
    ...
    {
        private const int DocumentDefaultLength = 11;
        ...
        public bool Validate(RegisterUserInput input)
        {
            ...
            Requires()
                ...
                .AreEquals(input.Document.Length, DocumentDefaultLength, DocumentLengthKey);

            Requires()
                .IsEnum(input.Role, (int)input.Role, RoleEnumRangeKey);
